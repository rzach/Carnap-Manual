...
In contrast to Carnap.io, Carnap Next determines the default settings for
`exam` and `feedback` based on the assignment type and whether grades
are released. On *Reading/resource* and *Practice assignments*,
`feedback` defaults to `full` and `exam` defaults to `false` (i.e.,
only correct exercises can be submitted on practice assignments). On
*Graded assignments*, `exam` defaults to `true` and `feedback` defaults to `none` until grades are
released, and to `full` thereafter. If an exercise has `exam` and/or
`feedback` explicitly specified, it overrides these default settings.
So a graded exercise that should give feedback or only accept correct
submissions has to say so on every exercise.
